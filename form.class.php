...
/**********************************
*            Form Class           *
* Originally created by Savageman *
* Improved by Pierre-EtienneCrépy *
*           License LGPL          *
**********************************/
...
class Form {
    ...
    public function __toString() {
        ...
        if (empty($this->fieldsets)) {
            ...
            $o .= $this->_html_fields("\t", array_diff_key($this->fields, $this->hidden_fields, $this->submit_fields));
            if (!empty($this->hidden_fields)) { $o .= $this->_html_hidden_fields("\t", $this->hidden_fields); }
            $o .= $this->_html_fields("\t", $this->submit_fields);
            ...
            if (!empty($hidden_fields)) { $o .= $this->_html_hidden_fields("\t", $hidden_fields); }
            if (!empty($submit_fields)) { $o .= $this->_html_fields("\t", $submit_fields); }
        }
        $o .= '<em>Les champs precedés de * sont obligatoire.</em></form>';
        return $o;
    }
    ...
    protected function _html_fieldset($tab, $legend, $fields) {
        ...
        $o .= $this->_html_fields("\t\t", $this->fields, $fields);
        $o .= "$tab\t".'</fieldset>'."\n";
        return $o;
    }
}
...
class Form_Text extends Form_Input {
    ...
    public function __toString() {
        ...
        $label = '';
        if (!empty($this->label)) {
			$obl = ($this->required == true) ? '*': null ;
            list($for, $id) = self::_generate_for_id($this->form->auto_id(), $this->attrs['name']);
            $label = '<label'.$for.'>'.$this->label.$this->form->label_suffix().' '.$obl.'</label>'."\n";
        }
        ...
        $field = '<input'.$id.$this->attrs.$value.$this->javascript.' />';
...
class Form_Captcha extends Form_Text {
    ...
    protected function _init() {
        ...
        if (!isset(self::$error_list['invalid_captcha'])) {

            self::$error_list['invalid_captcha'] = "La captcha entrée est incorrect";
        }
    }
    public function is_valid($value) {
        ...
        if (parent::is_valid($value)) {
            ...
            if (strtoupper($value) == $_SESSION['captcha']) {
				
				//session_unregister($_SESSION['captcha']);
                return true;
            }
            $this->_error('invalid_captcha');
            return false;
        }
        return false;
    }
	...
	$_SESSION['captcha'] = strtoupper(substr(md5(uniqid()),0,6));
	return $_SESSION['captcha'];
        ...
        if (!empty($this->label)) {
			$obl = ($this->required == true) ? '*': null ;
            list($for, $id) = self::_generate_for_id($this->form->auto_id(), $this->attrs['name']);
            $label = '<label'.$for.'>'.$this->label.$this->form->label_suffix().' <img src="captcha.php?captcha='.$this->generate_captcha().'" class="captcha" /> '.$obl.'</label>'."\n"; 
        }
...
class Form_File extends Form_Input {
        ...
        if (isset($_FILES[$name])) {
            ...
            if (parent::is_valid($value)) {
                ...
                if (!empty($this->extensions)) {
                    ...
                    $ext = pathinfo($value);
                    if (!in_array($ext['extension'], $this->extensions)) {
    ...
    protected function _custom_errors($id_error, $error) {
        ...
        if ('file_too_big' == $id_error) {
            ...
            $this->error_messages[$id_error] = vsprintf($error, $this->max_size);
            return true;
        }
        return false;
    }
...
class Form_Submit extends Form_Input {
    ...
    public function __toString() {
        ...
        $field = '<input'.$this->attrs.$value.$this->javascript.' class="submit"/>';
        return sprintf("%2\$s%1\$s", $field, $label);
    }
}
class Form_Button extends Form_Input {
    ...
    public function __toString() {
        ...
        // Pas d'auto_id pour les champs button...
        $label = (!empty($this->label)) ? '<label>'.$this->label.$this->form->label_suffix().'</label>'."\n$tab" : '';
        $value = empty($this->value) ? '' : ' value="'.$this->value.'"';

        $field = '<input'.$this->attrs.$value.$this->javascript.' />';
        return sprintf("%2\$s%1\$s", $field, $label);
    }
}
// ---------------------------------------------------------------------------
class Form_Radio extends Form_Input {
    ...
    public function __toString() {
        ...
        $field = "\n$tab".implode("\n$tab", $fields);
        return sprintf("%2\$s%3\$s%1\$s", $field, $span, $errors);
    }
}
...
class Form_Checkbox extends Form_Input {
    ...
    public function __toString() {
        ...
        if (!empty($this->label)) {
            ...
            list($for, $id) = self::_generate_for_id($this->form->auto_id(), $this->attrs['name']);
            $label = "\n".'<label'.$for.' style="display:inline;">'.$this->label.'</label>';
        }
        $errors = $this->error_messages->__toString();
        if (!empty($errors)) { $errors = "\n".$errors; }
       ...
       // $checked = ($this->value == $this->form->get_bounded_data($this->attrs['name'])) ? ' checked="checked"' : '';
        ...
        $field = '<input'.$id.$this->attrs.$value.$this->javascript.' />';
        return sprintf("%1\$s%2\$s%3\$s", $label, $field, $errors);
    }
}
...
class Form_Textarea extends Form_Field {
    ...
    public function __toString() {
        ...
        $field = '<textarea'.$id.$this->attrs.' '.$this->javascript.'>'.$value.'</textarea>';
        return sprintf("%2\$s%1\$s%3\$s", $field, $label, $errors);
    }
}
